...
class FightingModuleSite extends WeModuleSite {
	...
	public function doWebPlaylist() {
		...
		if (!empty($list)) {
			foreach($list as &$item) {
				...
				$fighting = pdo_fetch("SELECT title, fansnum, viewnum,starttime,endtime,isshow FROM ".tablename('fighting_reply')." WHERE rid = :rid ", array(':rid' => $item['id']));
	...
	public function doWebQuestions() {
		...
		if (checksubmit('delete') && !empty($_GPC['select'])) {
			...
			message('删除成功！', create_url('index/module', array('do' => 'detail', 'name' => 'fighting')));
		}
		$pindex = max(1, intval($_GPC['page']));
		$psize = 20;
	...
	public function doWebQuestioneditok(){
		...
		if ($_GPC['action'] = 'edit') {
			...
			message('修改成功！', create_url('index/module', array('do' => 'detail', 'name' => 'fighting')));
	...
	public function doWebQuestionaddok(){
		...
		if ($_GPC['action'] = 'add') {
			...
			message('增加成功！', create_url('index/module', array('do' => 'detail', 'name' => 'fighting')));
