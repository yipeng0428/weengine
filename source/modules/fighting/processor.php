	...
	public function respond() {
		...
		if(!$this->inContext) {
			...
			if (!$info['nickname']) {//判断是否绑定
				return $this->respText($row['reply1']."<a target=\"_blank\" href=\"mobile.php?act=channel&name=home&weid=1\">点击此处登记资料</a>，然后重新回复".$row['keyword']."开始答题");
			...
			if ( $content == '退出') {
        		...
        		function requireQuestions(){
        			$sql_question = "SELECT * FROM " . tablename('fighting_question_bank') . " WHERE id >= (SELECT floor( RAND() * ((SELECT MAX(id) FROM `ims_fighting_question_bank`)-(SELECT MIN(id) FROM `ims_fighting_question_bank`)) + (SELECT MIN(id) FROM `ims_fighting_question_bank`))) ORDER BY id LIMIT 1";
